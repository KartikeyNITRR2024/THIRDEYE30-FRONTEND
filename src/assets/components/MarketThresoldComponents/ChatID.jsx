...
export default function ChatID({ group, onBack }) {
  ...
  // Load chat IDs when component mounts or group changes
  useEffect(() => {
  ...
  const handleDelete = async (id) => {
    if (!group?.id) return;
    await deleteChatID(id, group.id); // pass group.id
  };
  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-md mt-6 max-w-full mx-auto relative">
      ...
      {/* Loading */}
      {loading && <p className="text-center text-gray-500 mb-2">Loading chat IDs...</p>}
              ...
              >
                <td className="border px-2 py-2">
                  <select
                    ...
                    onChange={(e) =>
                      setNewChat((prev) => ({ ...prev, workType: parseInt(e.target.value) }))
                    }
                  >
                ...
                <td className="border px-2 py-2">
                  <input
                    ...
                    className={`w-full text-center border rounded px-1 py-1 ${
                      !newChat.chatName ? "bg-gray-100" : "bg-white"
                    }`}
                  />
                </td>
                <td className="border px-2 py-2">
